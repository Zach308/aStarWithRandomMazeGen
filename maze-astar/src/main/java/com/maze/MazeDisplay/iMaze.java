package com.maze.MazeDisplay;

import com.maze.MazeGen.PassageTree;
import com.maze.Pathfinding.Astar;

import java.util.function.Consumer;

import static com.maze.MazeDisplay.Cell.Type.PATH;
import static com.maze.MazeDisplay.Cell.Type.WALL;
...
public class iMaze {
    ...
    private iMaze(int heightInCells, int widthInCells){
        ...
        }else if(heightInCells> 80 || widthInCells > 80){
    ...
    /*---ToString---*/
    /**
     * returns a string that is based off the created maze
     */
    private String createVisibleMaze(boolean showSolution){
        StringBuilder sb = new StringBuilder();
        for(Cell row[] : mazeAsGrid ){
            for(Cell cell : row){
                if(cell.isWall()){
                    sb.append("██");//██
                }else if(showSolution && cell.isGoal()){
                    sb.append("##");
                }else{
                    sb.append("  ");
                }
    ...
    /**
     * show the imaginary maze
     * to the user
     */
    @Override
    public String toString(){
        return createVisibleMaze(false);
    }
